...
# * USE blender --python _requirementsCheck.py
# ! DO NOT USE python _requirementsCheck.py unless you are sure you're referring to the correct python
...
_, lstDiffBool = listDiff(currPkgNames, reqPkgNames)
extraPkgs = [k for i, k in enumerate(currPkgs) if lstDiffBool[i]]
for pkgName in extraPkgs:
    thisCmd = 'pip uninstall ' + pkgName
    exCode = os.system(thisCmd)

# which packages changed versions?
verChangePkgs = list(set(extraPkgsIncVerChg) - set(extraPkgs)) # this has the old version number in it
...
print('Version changes (shows uninstalled version):') # list pakcages that changed version
print(verChangePkgs)
